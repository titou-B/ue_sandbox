...
void AROS2ActionServerNode::BeginPlay()
{
    ...
    // ROS2_CREATE_ACTION_SERVER_WITH_QOS( Node,
    //                                     this,
    //                                     ActionName,
    //                                     UROS2FibonacciAction::StaticClass(),
    //                                     &AROS2ActionServerNode::GoalCallback,
...
void AROS2ActionServerNode::Execute()
{
    ...
    if (Count++ <= GoalRequest.Order)
    {
        ...
        UE_LOG_WITH_INFO_NAMED(
            LogRos2SandBox, Log, TEXT("[%s][C++][update feedback] added %d"), *ActionName, FeedbackMsg.Sequence.Last(0));
    ...
    else
    {
        ...
        // Log request and response
        UE_LOG_WITH_INFO_NAMED(LogRos2SandBox, Log, TEXT("[%s][C++][send result] result is: %s"), *ActionName, *resultString);
        ;
    }
}
...
void AROS2ActionServerNode::GoalCallback(UROS2GenericAction* InAction)
{
    ...
    // Log request and response
    UE_LOG_WITH_INFO_NAMED(LogRos2SandBox, Log, TEXT("[%s][C++][goal callback]"), *ActionName);
}
...
void AROS2ActionServerNode::CancelCallback()
{
    ...
    // Log request and response
    UE_LOG_WITH_INFO_NAMED(LogRos2SandBox, Log, TEXT("[%s][C++][cancle callback]"), *ActionName);
}
...
void AROS2ActionServerNode::ResultCallback()
{
    ...
    // Log request and response
    UE_LOG_WITH_INFO_NAMED(LogRos2SandBox, Log, TEXT("[%s][C++][result callback] Start fibonacci calculation"), *ActionName);
}