...
void AROS2ServiceClientNode::BeginPlay()
{
    ...
    FTimerDelegate sendRequest = FTimerDelegate::CreateLambda(
        [this]
        {
            ...
            // Log request
            UE_LOG_WITH_INFO_NAMED(LogRos2SandBox, Log, TEXT("[%s][C++][send request] a:%d, b:%d"), *ServiceName, req.A, req.B);
        });
...
void AROS2ServiceClientNode::ReceiveResponse(UROS2GenericSrv* InService)
{
    ...
    // Log response
    UE_LOG_WITH_INFO_NAMED(LogRos2SandBox, Log, TEXT("[%s][C++][receive response] sum:%d"), *ServiceName, res.Sum);
}